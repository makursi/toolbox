...
- **Docs are repo-level or App-level by the ownership rule above.** At the root: `docs/adr/` — decisions worth not re-litigating — `docs/agents/` — how agents file issues and read these docs — and `CONTEXT.md`, the vocabulary. Inside the App: `apps/web/docs/design.md` (the UI: tokens, rules, pre-flight checklist) and `apps/web/docs/adding-a-tool.md` — the contract for the thing this repo does most: what a Tool has to pass, which files it touches, where its code lives, and the order the work happens in.
...
- **UI work follows `apps/web/docs/design.md`.** It holds the tokens, the rules that are not up for re-litigation, and the pre-flight checklist; update it in the same commit as the change. The visual language comes from the `minimalist-ui` protocol, the gates from `design-taste-frontend`.
...
- **Verify UI against the production build** (`pnpm build && pnpm start`), because `next dev` holds back hydration until its HMR origin is accepted, so the page renders and then ignores every click. `apps/web/docs/design.md` section 11 carries the method, the evidence and what is still unverified.
