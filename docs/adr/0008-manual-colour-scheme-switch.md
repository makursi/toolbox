...
ADR-0007 set the scheme to `auto` in both `ColorSchemeScript` and the provider, and `apps/web/docs/design.md` section 7 turned that into a rule: "跟随操作系统，**不做手动切换开关**". It held while the dark scheme had never been rendered; once both schemes had been seen in a browser, a dark scheme nobody can choose is a dark scheme nobody sees. The header now carries a two-state switch.
