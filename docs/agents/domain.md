...
- **`src/tools/<slug>/rules.md`**: when the work is inside one Tool and its own rules doc exists, read it first — it holds rules that bind only that Tool (`docs/adr/0014-tool-scoped-rules-live-with-the-tool.md`); the site's design doc set holds site-level rules only.
