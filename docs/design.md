...
| 资源      | 路径                                     | 接线位置                                                     |
| --------- | ---------------------------------------- | ------------------------------------------------------------ |
| 站点 logo | `apps/web/public/logo.svg`               | 页头，替换掉 `apps/web/src/app/layout.tsx` 里的文字 wordmark |
...
2. **logo 与封面图**：两张图到位后，页面才有标记、卡片才有配图。注意卡片上**已经不存在空图框**了（见第九节），所以这条是"等素材"，不再是"看着缺一块"。
