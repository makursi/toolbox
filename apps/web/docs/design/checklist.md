...
- [ ] **行为那一半不再手过**：真鼠标点每个可点控件（不只键盘）、一个工具的核心路径端到端（文件进、结果出，先例：image-converter 的五格式转换）、下载真的落盘且文件头是它名字声明的格式、零外发请求与零 console 报错——这些已经在 CI 上每次改动跑一遍（`pnpm e2e`；每个工具的 spec 与手工清单在它的 README 与 `e2e/`）。仍要手过的是工具 README 清单里剩下的部分：键盘与拖拽、队列与长任务、超限与坏文件、取消。**几何与两套配色的对照不在门禁里**——那是量具的活；门禁只读像素通道值（透明那一半是否还是透明、压平那一半是否纯白），那是别人文件里的像素，与界面配色不是一回事。
